...
c Subroutinen eos_input leser inn disse fra fil. 
c I tillegg setter den opp punkter på ny grid.
c Tilstandslikningen som blir brukt til betalikevekt blir lagret i
c common/eos/.
c Her kan du mate inn de ønskede verdier direkte uavhengig av eos_input.

c Programmet regner i tillegg ut trykk. Jeg er ikke helt trygg på
c den numeriske nøyaktigheten for trykket.  Det er nok følsomt for 
c hvilken tetthets grid  tilstandlikningen er beregnet på.

 
c input filen ser slik ut
c 21
c  .0346    .0492    .0675    .0899    .1167    
c  .1484    .1854    .2280    .2767    .3319    
c  .3939    .4633    .5404    .6256    .7192    
c  .8218    .9338   1.0554   1.3295   1.4828   1.6474
c 1.0
c  5.58     6.80     8.17     9.83    11.99    
c 15.07    19.64    26.55    36.77    51.73    
c 72.25    99.54   133.64   175.37   224.32   
c 281.02   344.80   417.02   618.23   762.07   894.63
c 0.0
c  8.49    -8.46    -9.68   -10.99   -12.21   
c -12.97   -12.71   -10.95    -6.78     1.05    
c  13.36    31.72    56.19    88.28   127.31   
c 173.75   227.88   288.36   441.34   550.26   670.48


      subroutine eos_input
      IMPLICIT REAL*8 (A-H,O-Z)
      common/eos/sym_e(1000),e_nuc(1000),dens(1000),ndens
      PARAMETER(maxd=1000)
      PARAMETER (ldi=1000)
      parameter (e=15.8, s=32., gamma=0.6)
      parameter (delta=0.2, rho0=0.16)
      dimension e_neu1(maxd),e_nuc1(maxd)
      dimension e_neu(1000)
      ...
      do j=1,ndens
         ...
         e_nuc(j)=erg
         e_neu(j)=erg+s*(x**gamma)
         write(6,*) xd, e_neu(j)-e_nuc(j)
      ...
      common/eos/sym_e(1000),e_nuc(1000),dens1(1000),ndens
      DIMENSION CHEMP(1000),CHEMN(1000)
      DIMENSION YNEU(1000),CHEM(1000),EN(1000)
     :         ,EM(1000),EE(1000),ETOT(1000)
     :         ,y_el(1000),dmass(1000),dnum(1000),edens(1000) 
      dimension x1(1),fx1(1),sqq(1000),sau(1000)
      character*9 cc
...
C***** STORE DATA ****************            
         ...
         WRITE(6,'(14E12.4)')BARYON/HC3,dmass(ib),1.d0-X,xelec,un,
...
C*********** NEXT DENSITY    ********************
      ...
      enddo
 4    format(E12.6,2x,e12.6)
 3    FORMAT(f8.4,6e13.5,2f8.4)
 2    FORMAT(5F10.4)
 1    FORMAT(d8.4,d8.4, d8.4,d8.4)
 6    FORMAT(d8.4,d8.4,d8.4)
...
c      calc d(nE)/dn
      ...
      IMPLICIT REAL*8 (A-H,O-Z)
      real*8 df(1),xx(1),a(1000),b(1000),f(n),x(n)
      xx(1)=x0
      ...
      IMPLICIT REAL*8 (A-H,O-Z)
      real*8 ff(1),xx(1),a(1000),b(1000),f(n),x(n)
      xx(1)=x0
...
10    IF(ju-jl.gt.1) THEN 
      ...
      IMPLICIT REAL*8 (A-H,O-Z) 
      DIMENSION xa(n),ya(n),c(1000),d(1000)
      PARAMETER (NMAX=101)
      ns=1
      dif=dabs(x-xa(1))
      DO i=1,n
