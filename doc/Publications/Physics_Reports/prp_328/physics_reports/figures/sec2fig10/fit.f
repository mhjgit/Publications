...
! 
            ...
            press=0.
            DO i=1,number_terms
               press=press+FLOAT(i+1)/3.0*polynom_terms(i)*(x**(FLOAT(i+4)/3.))
            ENDDO
...
!
            ...
            dpdn=0.
            DO i=1,number_terms
                  dpdn=dpdn+FLOAT(i+1)*FLOAT(i+4)/9.*polynom_terms(i)* &
                  (x**(FLOAT(i+1)/3.))
            ENDDO
...
!
      ...
      WRITE(7,*) ' Quality of energy fit:'
      WRITE(7,*) ' Number of terms in polynomial expansion:',  number_terms
      DO i=1,number_terms
         WRITE (7,*) i, polynom_terms(i)
      ENDDO
      WRITE (7,'(''CHISQ = '',F12.5)') chisq
      IF(chisq > 0.1) THEN
         WRITE(7,*) 'chi-square tolerance too high, try with more terms'
      ENDIF
...
!      WRITE(6,*) 'Density, e-fitted, original e, fitted-p, incompressibility, speed of sound/c'
      ...
      DO i=1,100
         x=i*0.01
         kappa=9*dpdn(x)
         adiabatic=x*dpdn(x)/press(x)
         IF (kappa > 0. ) v_over_c=SQRT(kappa/9./938.926)
         WRITE(6,'(E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4)') x, rho(x), press(x),kappa, v_over_c, adiabatic 
      ENDDO
      ...
      DOUBLE PRECISION :: chisq,a(ma),sig(ndata),u(mp,np),v(np,np),w(np), &
                          x(ndata),  y(ndata),TOL
      PARAMETER (NMAX=5000,MMAX=200,TOL=1.e-5)
      INTEGER :: i,j
