...
!
            FUNCTION rho(x)
            IMPLICIT NONE
            DOUBLE PRECISION :: rho, y, y1
            DOUBLE PRECISION,  INTENT(IN) :: x
            INTEGER :: i
            y=x/0.16
            rho= polynom_terms(1)*((2.+0.2)*y/(1.+0.2*y)-y*y/(1.+0.2*y))
!                -polynom_terms(2)*y*y/(1.+0.2*y)
            rho= -16.*((2.+0.2)*y/(1.+0.2*y)-y*y/(1.+0.2*y))
...
!
      ...
      DOUBLE PRECISION :: chisq, kappa, v_over_c, adiabatic, x
      EXTERNAL eos_param
...
!         e(i)=e(i)*n(i)+938.926*n(i)
      ...
      DO i=1,number_of_data
         ...
         IF (kappa > 0. ) v_over_c=SQRT(kappa/9./938.926)
         WRITE(6,'(E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4)') n(i), rho(n(i)), e(i),press(n(i)),kappa, adiabatic, v_over_c 
         WRITE(6,'(E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4)') n(i), rho(n(i)), e(i)
      ENDDO


      DO i=0,300
         x=i*0.01
         WRITE(6,'(E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4)') x, rho(x)
      ENDDO
      ...
      IMPLICIT NONE
      INTEGER :: ma, i
      DOUBLE PRECISION :: afunc, x, y
      DIMENSION afunc(ma)
      y=x/0.16
      afunc(1)=(2.+0.2)*y/(1.+0.2*y)-y*y/(1.+0.2*y)
!      afunc(2)=-y*y/(1.+0.16*y)
