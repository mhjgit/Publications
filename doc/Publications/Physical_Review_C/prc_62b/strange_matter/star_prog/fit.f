...
!
            ...
            DOUBLE PRECISION,  INTENT(IN) :: x
            INTEGER :: i
            drho=polynom_terms(2)
            DO i=3,number_terms
               drho=drho+FLOAT(i-1)*polynom_terms(i)*(x**(FLOAT(i-2))) 
            ENDDO
...
!
            ...
            DOUBLE PRECISION,  INTENT(IN) :: x
            INTEGER :: i
            ddrho=2.*polynom_terms(3)
            DO i=4,number_terms
               ddrho=ddrho+FLOAT((i-1)*(i-2))*polynom_terms(i)*&
                           (x**(FLOAT(i-3))) 
            ENDDO
...
!
      ...
      DOUBLE PRECISION :: chisq, kappa, v_over_c, adiabatic,xx, yy, zz
      EXTERNAL eos_param
...
!v_over_c=SQRT(kappa/9./938.926)
!         v_over_c=kappa/9./938.926
         WRITE(6,'(E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4,2X,E10.4)') n(i), rho(n(i)), e(i),press(n(i)), &
                    kappa, adiabatic, v_over_c 
      ENDDO
