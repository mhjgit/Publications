...
\pacs{ PACS Numbers: 21.80.+a, 13.75.Ev, 21.65.+f. \\ 
  Keywords: Hypernuclei, $YN$ interaction, $G$-matrix, self--energy. }
\bigskip
\newpage
...
%$\varepsilon_{Y}(\infty)$, is simply given by \\
%\begin{equation}
%\varepsilon_{Y}(\infty)=-U_{Y}(p_{Y}=0)
%\label{eq:bin}
%\end{equation}
...
%which includes the corrections from the finite nucleus,
%is given by Eq.\ (\ref{eq:2p1h}) below while  the third term is given in
%Eq.\ (\ref{eq:vc}).
...
{}From Eqs. (\ref{eq:hf}), (\ref{eq:2p1h}) and (\ref{eq:vc}) we see that
...
As described before, our method provides the single-particle energies
...
Let us first show, in Fig. \ref{fig:binener}, the
...
In Tables \ref{tab:olambda} and \ref{tab:osigma} we show the
single-particle
...
%\cite{yama85,yama90,yama92,yama94}
...
For completeness, we have also calculated the width of the $\Sigma$
...
We now want to explore if our $\Lambda$ single-particle
...
(1972) 210
\bibitem{schulze} H.-J. Schulze, M. Baldo, U. Lombardo, J. Cugnon and
